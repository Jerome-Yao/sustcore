 ...
 * @param thread 线程能力. 如果是INVALID_CAP_IDX, 则表示让出当前进程
 */
 ...
 */
void wakeup_process(CapIdx pid);
 ...
 *
 * @param size 共享内存大小
 * @return CapIdx 共享内存能力
 */
CapIdx makesharedmem(size_t size);
 ...
 * @param shmid 共享内存能力
 */
void sharemem_with(CapIdx pid, CapIdx shmid);
 ...
 * @param cap 共享内存能力
 */
void freesharedmem(CapIdx cap);

/**
 * @brief 申请物理内存
 *
 * @param size 物理内存大小
 * @return CapIdx 物理内存能力
 */
CapIdx require_phymem(size_t size);
 ...
 * @param size 消息大小
 */
void send_message(CapIdx pid, const void *msg, size_t size);

...
VFS (Virtual File System) 是 Sustcore 内核的虚拟文件系统层。它为上层应用提供统一的文件操作接口, 同时将具体的文件系统实现 (tarfs, ext4, devfs 等) 作为可插拔驱动接入。VFS 的设计与内核的 Capability 模型深度集成: 文件、目录和挂载点均以 Capability Payload 的形式暴露给用户态。
...
inode 编号只在同一个 superblock 内稳定唯一, 具体含义由文件系统实现决定。例如, tarfs 使用 `tar header offset / 512` 作为 inode id。
...
| 接口          | 职责                                                                                  |
| ------------- | ------------------------------------------------------------------------------------- |
| `IMetadata`   | 元数据空基类, 生命周期由提供者管理                                                    |
| `IINode`      | inode 通用接口, 支持 `FILE` / `DIRECTORY` / `SYMLINK` RTTI                            |
| `IFile`       | 普通文件读写: `read(offset, buf, len)`, `write(offset, buf, len)`, `size()`, `sync()` |
| `IDirectory`  | 目录操作: `lookup(name)`, `mkfile(name)`, `mkdir(name)`, `entry_count()`              |
| `ISuperblock` | 超级块: `get_inode(inode_id)`, `sync()`, `flush_file_pages()`                         |
| `IFsDriver`   | 文件系统驱动: `probe(device, options) → ISuperblock*`                                 |

此外还有 `ISymlink` (符号链接, 提供 `target()`) 和 `IPesudoFsDriver` (伪文件系统驱动, 不依赖块设备, 如 devfs / procfs / tmpfs)。
...
VFS 运行时层 (`VFS` 单例) 对文件系统接口层返回的裸指针进行引用计数包装, 形成安全的对象生命周期管理。

### VFsDriver

`VFsDriver` 是 `IFsDriver` 的引用计数包装。它持有 `util::owner<IFsDriver *>`, 通过 `closable()` (当引用计数归零时返回 true) 阻止仍被挂载 superblock 引用的文件系统驱动被卸载。
...
当 inode 内容变化时, VFS 使用 `invalidate_inode()` 原地刷新已缓存的 `VINode` (替换内部 `_inode`), 保持 vnode 指针地址稳定。当 inode 被删除时, 使用 `evict_inode()` 从缓存中移除。
...
`VINode` 还实现了文件页缓存 (`CachedFilePage` 链表), 用于 file-backed memory 映射时的页面管理。每个缓存页包含:
...
`VFile` 是 `PayloadType::VFILE` 的 Capability Payload, 持有 `VINode` / 挂载路径 / VFS 引用。析构时通知 VFS 当前挂载点少了一个活跃文件 —— 这是 `VFS::umount()` 判断 busy 状态的基础。
...
内核采用 Capability-based 访问控制: 要获取 VFile 或 VDir, **必须**指定其父目录的 Capability 和相对路径，下图展示 init VDir 的来源
...
kinit 通过特殊接口获得根目录的 Capability, 传递给 init 进程。init 通过该 Capability 获取根目录下的文件和目录的 Capability, 再传递给子进程。这就形成了 Capability 的传递链, 保证资源分配是受限的: 如果某个目录下的文件没有被传递给某个进程, 该进程就无法访问。
...
路径解析时, VFS 先根据全局路径在挂载表中找到匹配的挂载点, 然后将余下部分在 `VSuperblock` 的根 inode 下逐级 lookup。

卸载时, VFS 首先检查是否存在活跃的 VFile/VDirectory 引用了该挂载点下的 inode (通过引用计数), 若 busy 则拒绝卸载并返回 `ErrCode::BUSY`。

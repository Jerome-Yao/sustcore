...
本文总览 Sustcore 当前使用的容器工具。相关实现位于 `include/sus/list.h`、`include/sus/map.h`、`include/sus/queue.h`、`include/sus/tree.h` 以及 `include/std/c++/` 下的标准库兼容实现。
...
| `util::StaticArrayQueue` | `include/sus/queue.h` | 固定容量环形队列 | 拥有静态数组空间 |
...
- `StaticArrayQueue::push()` 在满队列时返回 `OVERFLOW_ERROR`
- `StaticArrayQueue::pop()` 在空队列时返回 `UNDERFLOW_ERROR`

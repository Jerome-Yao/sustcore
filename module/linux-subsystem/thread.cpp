...
CapIdx linuxss_create_thread(addr_t entrypoint) {
    if (entrypoint == 0) {
        printf("linux-subsystem: create thread failed, entrypoint is null\n");
        return cap::error;
    }

    void *stack_base = linuxss_alloc_stack(LINUXSS_THREAD_STACK_SIZE);
    if (stack_base == nullptr) {
        printf("linux-subsystem: create thread failed, stack alloc failed\n");
        return cap::error;
    }
    ...
    if (tcb_cap == cap::null || tcb_cap == cap::error) {
        printf("linux-subsystem: create thread syscall failed entry=%p\n",
               reinterpret_cast<void *>(entrypoint));
        return cap::error;
    }

    printf(
        "linux-subsystem: create thread placeholder entry=%p stack=[%p,%p)\n",
        reinterpret_cast<void *>(entrypoint), stack_base, stack_top);

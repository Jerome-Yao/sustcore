...
void BuddyAllocator::post_init() {
    ...
    for (int i = 0; i <= BuddyAllocator::MAX_BUDDY_ORDER; i++) {
        ...
        // 我们将其转换为 KA
        FreeBlock *head_ka = (FreeBlock *)PA2KPA(head);
        FreeBlock *tail_ka = (FreeBlock *)PA2KPA(tail);

        sentinel_kva->next = head_ka;
        sentinel_kva->prev = tail_ka;

        // 遍历并修复所有节点
        FreeBlock *iter = sentinel_kva->next;

        while (iter != sentinel_kva) {
            // iter 现在是 KA
            // iter->next 和 iter->prev 在内存中仍然是 PA
            FreeBlock *next_pa = iter->next;
            FreeBlock *prev_pa = iter->prev;

            FreeBlock *next_ka = (FreeBlock *)PA2KA((umb_t)next_pa);
            FreeBlock *prev_ka = (FreeBlock *)PA2KA((umb_t)prev_pa);
